...
- **Prebuild**: `copy-archive-images.mjs` copies `images/` → `public/archive-images/`
...
- `public/` — Static assets (logos, icons); `archive-images/` is gitignored (generated at build)
- `archive/` — Scraped newsletter data (committed). Each issue: `metadata.json` + `index.html`
- `images/` — Deduplicated images by SHA-256 hash (committed). Referenced from `metadata.json` as `./hash.ext`
...
### Important Notes
- `archive/` and `images/` are committed to the repo (not gitignored)
- `public/archive-images/` is gitignored — generated by the prebuild script
